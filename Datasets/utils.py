...
class CropCenter(object):
    ...
    def __call__(self, sample):
        ...
        scale_h, scale_w, scale = 1.0, 1.0, 1.0
        if th > h:
            scale_h = float(th) / h
        if tw > w:
            scale_w = float(tw) / w
...
class ToTensor(object):
    def __call__(self, sample):
        ...
        for kk in kks:
            ...
            if (
                len(data.shape) == 3 and data.shape[0] == 3
...
def dataset_intrinsics(dataset="tartanair"):
    if dataset == "kitti":
        ...
        # focalx, focaly, centerx, centery = 320.0, 320.0, 320.0, 240.0 # 640 x 480
        focalx, focaly, centerx, centery = 320.0, 320.0, 320.0, 320.0  # 640 x 640
    else:
        return None
...
def load_kiiti_intrinsics(filename):
    ...
    focalx, focaly, centerx, centery = (
        float(cam_intrinsics[0]),
        float(cam_intrinsics[5]),
        float(cam_intrinsics[2]),
        float(cam_intrinsics[6]),
    )
